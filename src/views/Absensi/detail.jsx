...
export default function AbsensiDetail() {
  ...
  useEffect(() => {
    ...
    axiosClient
      .get(`/absensi-detail/${decryptedId}`)
  ...
  const handleConfirm = () => {
    ...
    axiosClient
      .get(`/absensi-confirm/${absensi.id}`, { hr_status: "approved" })
      .then(() => axiosClient.get(`/absensi-detail/${absensi.id}`))
      .then((res) => setAbsensi(res.data?.data))
      .catch(() => setError("Gagal mengubah status HR"))
  ...
  const showConfirmBtn = absensi?.hr_status?.toLowerCase() === "pending";
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="bg-white shadow-lg rounded-2xl p-8 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-gray-700 text-sm">
          ...
          <DetailItem label="Catatan HR" value={absensi?.hr_note} />
          <DetailItem label="Keterangan" value={absensi?.keterangan} />
          <DetailItem label="Lokasi" value={absensi?.latitude && absensi?.longitude ? `${absensi.latitude}, ${absensi.longitude}` : "-"} />
        </div>

        {/* Tombol Kembali & Approved */}
        {(showConfirmBtn || true) && (
          <div className="mt-10 flex justify-end space-x-4">
            ...
            {showConfirmBtn && (
              <button onClick={handleConfirm} disabled={submitting} className={`px-6 py-2 rounded-lg text-white text-sm transition ${submitting ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}>
                {submitting ? "Memproses…" : "Approved"}
              </button>
            )}
          </div>
        )}
